...
def get_horiz(grid, x, y):
	prod = True
	if x < 3:
		prod = False
	else:
		prod = int(grid[y][x-3]) * int(grid[y][x-2])  * int(grid[y][x-1]) * int(grid[y][x]) 
	return prod

def get_vert(grid, x, y):
	prod = True
	if y < 3:
		prod = False
	else:
		prod = int(grid[y-3][x]) * int(grid[y-2][x])  * int(grid[y-1][x]) * int(grid[y][x]) 
	return prod

def get_r_diag(grid, x, y):
	prod = True
	if x > 16 or y < 3:
		prod = False
	else:
		prod = int(grid[y-3][x+3]) * int(grid[y-2][x+2])  * int(grid[y-1][x+1]) * int(grid[y][x]) 
	return prod	


def get_l_diag(grid, x, y):
	prod = True
	if x < 3 or y < 3:
		prod = False
	else:
...
def get_largest_product(grid, x, y):
	results = []
	results.append(get_horiz(grid, x, y))
	results.append(get_vert(grid, x, y))
	results.append(get_r_diag(grid, x, y))
	results.append(get_l_diag(grid, x, y))
...
def main():
	...
	for x in xrange(grid_x):
		for y in xrange(grid_y):
			cur_largest = get_largest_product(grid, x, y)
